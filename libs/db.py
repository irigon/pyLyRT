...
class DB:
    def __init__(self, table_name):
        ...
        self.dbconn.execute("CREATE TABLE " + self.name + " (" + db_args + ")")
        self.dbconn.execute('CREATE INDEX search_order ON ' + self.name + ' \
    ...
    def player_that_play(self, role, compartment=None):
        pass

    def add_relation(self, compartmentId, coreId, playerId, roleId, relationType, level, sequence):
        self.dbconn.execute("INSERT INTO roles_db VALUES('{}', '{}', '{}' , '{}', '{}', {}, {})".format(\
                            compartmentId, coreId, playerId, roleId, relationType, level, sequence))
    ...
    def find_next_level_and_sequence(self, core, player, role, type):
        ...
        if type=='PPR':
            ...
            result = self.dbconn.execute("SELECT BindingSequence FROM {} WHERE CorePlayerId='{}' and PlayerId='{}' \
            ...
            result = self.dbconn.execute("SELECT BindingLevel FROM {} WHERE CorePlayerId='{}' and RoleId='{}' \
            ...
            result = self.dbconn.execute("SELECT BindingSequence FROM {} WHERE CorePlayerId='{}' and BindingLevel='{}' \
                                              order by BindingLevel DESC,BindingSequence DESC".format \
    ...
    def unbind(self, core_id, role_id):
        ...
        result = self.dbconn.execute("DELETE FROM {} WHERE CorePlayerId='{}' and RoleId='{}'". \
                                     format(self.name, core_id, role_id))
        queue=[role_id]
        while len(queue) > 0:
            role=queue.pop()
            result = self.dbconn.execute("SELECT * FROM {} WHERE CorePlayerId='{}' and PlayerId='{}'". \
                                         format(self.name, core_id, role)).fetchall()
            queue.extend([x[3] for x in result])
            result = self.dbconn.execute("DELETE FROM {} WHERE CorePlayerId='{}' and PlayerId='{}'".\
